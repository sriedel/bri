...
Now with ruby 1.9 and the rdoc 2.x/3.x gem ri is very responsive. However the
output format looks like darkfish without the HTML and uses up a lot of space.
And subjectively, it's ugly.

This is where bri comes in: it tries to emulate fastri/qris output format and
lookup patterns while using the new rdoc 2.x/3.x infrastructure.
